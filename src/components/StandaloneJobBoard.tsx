...
interface StandaloneJobBoardProps {
  /** Custom title for the job board */
  title?: string;
  ...
  showHero?: boolean;
  /** Custom job data (replaces mock data) */
  jobs?: Job[];
  /** Callback when a job application is submitted */
...
export default function StandaloneJobBoard({
  ...
  const handleApplicationSubmit = (applicationData: any) => {
    if (selectedJob && onJobApplication) {
      onJobApplication(selectedJob.id, applicationData);
    }
  };
  ...
  const containerStyle = brandColors ? {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 ${className}`} style={containerStyle}>
      {/* Hero Section */}
      {showHero && (
